...
import timm
from core_export.submodule import *
from core_export.fusion import *
from core_export.mobilevit_export import replace_mobilevit_blocks
...
class ContextExtractor(SubModule):
    def __init__(self):
        ...
        model = timm.create_model('mobilevitv2_200.cvnets_in1k', pretrained=True, features_only=True, exportable=False)
        # 需要替换MobileVitV2Block为动态尺寸版本, 否则报错:
        ...
        #  Name:'/context_net/encoder_block3/encoder_block3.1/Reshape'
        #  Status Message: D:\a\_work\1\s\onnxruntime\core/providers/cpu/tensor/reshape_helper.h:41
        #  onnxruntime::ReshapeHelper::ReshapeHelper size != 0 && (input_shape_size % size) == 0 was false.
        #  The input tensor cannot be reshaped to the requested shape.
        #  Input shape:{1,256,124,180}, requested shape:{-1,256,46,2,80,2}
        model = replace_mobilevit_blocks(model)
