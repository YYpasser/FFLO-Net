...
from core.FFLONet import FFLONet
from core.utils.utils import InputPadder, count_parameters
...
def demo(args):
    ...
    with torch.no_grad():
        ...
        for (imfile1, imfile2) in tqdm(list(zip(left_images, right_images))):
            ...
            plt.imsave(output_directory / f"{file_stem}_sf.png", disp.squeeze(), cmap='jet')

    logging.info(f"Saving file {output_directory.absolute()}.")

if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument('--restore_ckpt', default='./pretrained_models/sceneflow/FFLONet.pth', help="load the weights from a specific checkpoint")
