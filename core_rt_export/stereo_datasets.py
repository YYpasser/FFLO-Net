...
from core_rt_export.utils import frame_utils
from core_rt_export.utils.augmentor import FlowAugmentor, SparseFlowAugmentor
...
class StereoDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        flow = flow[:1]
        return self.image_list[index] + [self.disparity_list[index]], img1.clone(), img2.clone(), flow.clone(), valid.float().clone()


    def __mul__(self, v): # 乘法运算, 扩充数据集
...
class SceneFlowDatasets(StereoDataset):
    def __init__(self, aug_params=None, root='F:/Datasets/stereo/SceneFlow', dstype='frames_finalpass', things_test=False):
...
class KITTI(StereoDataset):
    def __init__(self, aug_params=None, root='F:/Datasets/stereo/KITTI', image_set='training'):
        super(KITTI, self).__init__(aug_params, sparse=True, reader=frame_utils.readDispKITTI)
        assert os.path.exists(root)

        root_12 = 'F:/Datasets/stereo/KITTI/KITTI2012'
        ...
        root_15 = 'F:/Datasets/stereo/KITTI/KITTI2015'
...
class MiddleburyXXXX(StereoDataset):
    def __init__(self, aug_params=None, root='E:/StereoDataset/Middlebury/Middlebury2014', split='F'):
        super(Middlebury, self).__init__(aug_params, sparse=True, reader=frame_utils.readDispMiddlebury)
        assert os.path.exists(root)
        assert split in "FHQ"
        lines = list(map(osp.basename, glob(os.path.join(root, "trainingH/*"))))
        ...
        image1_list = sorted([os.path.join(root, f'training{split}', f'{name}/im0.png') for name in lines])
        image2_list = sorted([os.path.join(root, f'training{split}', f'{name}/im1.png') for name in lines])
        disp_list = sorted([os.path.join(root, f'training{split}', f'{name}/disp0.pfm') for name in lines])
...
def fetch_dataloader(args):
    ...
    for dataset_name in args.train_datasets:
        if re.compile("middlebury_.*").fullmatch(dataset_name): # Middlebury数据集
            # re.compile()编译正则表达式, fullmatch()检查字符串是否完全匹配正则表达式
            # new_dataset = Middlebury(aug_params, split=dataset_name.replace('middlebury_',''))
            # 在线评估数据集
            eval3_dataset = Middlebury(aug_params, split=dataset_name.replace('middlebury_',''))
            # Middlebury2014数据集
            middlebury2014 = MiddleburyXXXX(aug_params, split=dataset_name.replace('middlebury_',''))
            # Middlebury2021数据集
            middlebury2021 = MiddleburyXXXX(aug_params, split=dataset_name.replace('middlebury_',''))
            new_dataset = eval3_dataset + middlebury2014 + middlebury2021
            logging.info(f"Adding {len(new_dataset)} samples from Midllebury")
        elif dataset_name == 'sceneflow': # SceneFlow数据集
            # cleanpass 子集
        ...
        elif 'kitti' in dataset_name: # KITTI数据集
